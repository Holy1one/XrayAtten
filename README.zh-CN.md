...
precision 只影响补充网格点密度，不会删除原始 NIST 点或吸收边行。`energy_range_kev` 会从该 precision 网格中按闭区间筛选输出点，不额外插入任意边界点；`energies_kev` 列表则只输出用户指定点。可选值为：
...
- `coefficients`：默认输出完整 precision coefficient grid，也支持用 `energies_kev` 指定少数能量点，或用 `energy_range_kev` 输出某个能量范围内的 precision 格点。每个材料分别输出总衰减系数和近似能量吸收系数；有密度时同时输出线性系数。`online_comparison: true` 时会先写出 local 结果，再尝试 online XCOM 对照；若 online 失败，错误信息会明确说明 local 输出已经完成。
...
该 workflow 可生成完整材料系数曲线，也可输出指定能量点或指定能量范围内的材料衰减/吸收系数。范围模式严格使用 `precision` 生成的格点并进行范围筛选，不会随意增加额外插值点。
...
energies_kev: null                  # null 输出完整 precision dense grid；也可填 [10, 20]
# energy_range_kev:                 # 可选范围模式；与非 null energies_kev 互斥
#   start: 10
#   stop: 100
...
| `energies_kev` | `coefficients` 中可为 `null` 或非空指定点列表；厚度衰减、首碰吸收和多层剖面中必须为非空 keV 列表 |
| `energy_range_kev` | `coefficients` 可选 `{start, stop}` 范围；与非 null `energies_kev` 互斥；按 `precision` 网格筛选 |
